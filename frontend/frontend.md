...
1. **HTML** (HyperText Markup Language) — Скелет и DOM. HTML — это язык разметки, который определяет структуру и семантику документа. Он говорит браузеру: «Это заголовок», «Это параграф», «Это кнопка». Когда браузер получает HTML-файл, он не просто отображает его. Он проходит этап Парсинга (Parsing):
- Браузер читает байты HTML.
- Преобразует их в токены (теги, атрибуты, текст).
- Строит DOM (Document Object Model) — древовидную структуру объектов в памяти.
...
2. **CSS (Cascading Style Sheets)** — Внешний вид и CSSOM. CSS отвечает за визуальное представление элементов DOM: цвета, шрифты, отступы, расположение (layout), анимации.
Процесс обработки CSS сложнее, чем кажется:
-  Браузер скачивает CSS-файлы (или читает встроенные стили).
- Парсит их и строит CSSOM (CSS Object Model).
- CSSOM — это тоже дерево, но оно отражает иерархию стилей и их специфичность (вес селекторов).

3. **JavaScript** — Интерактивность и управление. JavaScript (JS) — язык программирования, который позволяет изменять DOM и CSSOM динамически, реагировать на действия пользователя (клики, ввод) и общаться с сервером (API).

4. **DOM** (Document Object Model) — программный интерфейс к HTML
5. **SPA** (Single Page Application) — приложение загружается один раз, далее динамическое обновление
